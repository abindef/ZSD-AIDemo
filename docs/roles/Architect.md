...
| **架构设计** | 定义分层架构、技术选型、集成方案 | 架构设计文档、脚手架 | [项目脚手架](../architecture/scaffold.md) |
...
| 规则 | 说明 | 架构影响 |
|------|------|----------|
| 认款类型互斥 | 同一认款单只能认款到一种类型 | 聚合内不变量 |
| 冲销条件 | 同项目、同业务单元、同供应商、同公司 | 领域服务校验 |
| 冲销优先级 | 预付 > 入库 > 销售 > 回款 | 策略模式 |
| 金额精度 | 含税2位、不含税10位 | 值对象封装 |
| 盘点期间限制 | 盘点期间不可认款 | 应用服务校验 |
| 最终一致性 | 跨服务事务采用Saga模式 | 事件驱动架构 |
